...
//           <MainHeading title="Top Workspaces" />
...
//           {data?.data?.slice(0, 8).map((space: any) => (
//             <div
//               key={space._id}
//               className="group border overflow-hidden rounded-xl bg-white shadow-sm shadow-slate-200/70 transition-all duration-300 "
//             >
//               <div className="relative h-56 w-full overflow-hidden">
//                 <img
...
//               <div className="p-5">
//                 <h3 className="text-lg line-clamp-1 font-bold text-slate-900 transition-colors duration-300 group-hover:text-primary">
//                   {space.name}
//                 </h3>

//                 <p className="mt-1 flex items-center gap-1 text-sm text-slate-500">
//                   <FiMapPin className="h-3.5 w-3.5" />
//                   {space.subCategory?.name}, {space.category?.name}
//                 </p>
...
//                       ?.toLowerCase()
//                       .replace(/\s+/g, "-")}/${space.slug}`}
//                     className="group/btn flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white text-[14px] transition-colors duration-300 hover:bg-primary"
//                   >
//                     Get quote
//                     {/* <FiArrowRight className="h-3.5 w-3.5 transition-transform duration-300 group-hover/btn:translate-x-1" /> */}
//                   </Link>
  ...
  return (
    <section className="bg-[#FBF7EF] px-6 py-16 sm:py-12">
      <div className="mx-auto max-w-7xl">
        ...
        >
          {data?.data?.map((space: any) => (
            <SwiperSlide key={space._id} className="mb-3">
              <div className="group overflow-hidden rounded-2xl bg-white shadow-sm shadow-slate-200/70 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-slate-300/60">
                <div className="relative h-56 w-full overflow-hidden">
                  <Swiper modules={[Navigation]} navigation loop>
                    {space.images?.map((img: any, index: number) => (
